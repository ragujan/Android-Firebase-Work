...
import com.rag.firebaseauthentication.util.StringUtils;
...
//                        if the unique image name is not empty means, new image is chosen, so we have to update the image too
    ...
    public void loadData() {
        ...
        if (extras != null) {
            ...
            Disposable disposable = FoodListRetrievalV3.getSingleFoodItem(foodItemDocumentId).observeOn(AndroidSchedulers.mainThread()).subscribe(dataMap -> {
    ...
    private void getBundle(FoodDomainRetrieval foodDomain) {
        ...
        foodPrice.setText(foodDomain.getPrice());
